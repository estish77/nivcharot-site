...
/**
 * The site's social brand glyphs, in one place.
 *
 * These paths already existed inline in `Footer` and (for X/TikTok) in the
 * podcast page's `PlatformLinksRow`; the 2026-08-27 brief asked for social
 * links on the media page too, which would have made a third hand-copied
 * set of the same 1-2KB path strings. They live here now instead, and
 * `Footer` imports them rather than carrying its own copies.
 *
 * No icon package is pulled in for this: these ARE the standard, recognizable
 * brand marks, they're already the site's icon vocabulary, and every other
 * glyph in this codebase (`PressTypeIcon`, `NavMenu`'s close, `Carousel`'s
 * chevron) is hand-drawn inline in the same way. Adding FontAwesome or
 * Lucide would ship a dependency to duplicate what's already here — and
 * neither ships the brand marks in its free tier anyway.
 */
...
export const socialIconPaths: Record<SocialNetwork, ReactNode> = {
  facebook: (
...
export type SocialLinkItem = {
  ...
  href: string
  /** Accessible name — the network on its own is ambiguous when a page links two different accounts on the same platform. */
  label: string
}

export type SocialLinksRowProps = {
  links: SocialLinkItem[]
  /** Visible lead-in text, e.g. "עקבו אחרינו". Rendered as the row's label. */
  heading?: ReactNode
  className?: string
}

/**
 * A labelled row of social icon links. Used on the media page (2026-08-27
 * brief) under the page title, in the site's flat style: 2px-bordered
 * square targets, brand red on hover, no radii.
 */
export function SocialLinksRow({ links, heading, className }: SocialLinksRowProps) {
  return (
