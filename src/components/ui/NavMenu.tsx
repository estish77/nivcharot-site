        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              initial={shouldReduceMotion ? false : { y: '-105%', opacity: 0, rotate: 0 }}
              animate={{ y: 0, opacity: 1, rotate: 0 }}
              exit={shouldReduceMotion ? { opacity: 0 } : { y: '-105%', opacity: 0 }}
              whileHover={
                shouldReduceMotion
                  ? undefined
                  : { rotate: [0, -1.3, 1.1, -0.6, 0], transition: { duration: 1.8, ease: 'easeInOut' } }
              }
              transition={shouldReduceMotion ? { duration: 0 } : { duration: 0.85, ease: EASE }}
            >
              <button
                type="button"
                onClick={() => setOpen(false)}
